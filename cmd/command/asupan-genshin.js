let handler = async (m, { conn } = {}) => {
  ...
  try {
    ...
    const url = gensin[Math.floor(Math.random() * gensin.length)]
    if (!url) throw new Error('No image available')
    
    await m.reply('⏳ Loading Genshin Impact waifu...')
    await conn.sendFile(m.chat, url, 'gensin.jpg', `
✨ *GENSIN IMPACT WAIFU*
_Nih Kak_ 💖`, m)
    
  } catch (error) {
    console.error('Genshin error:', error)
    m.reply('❌ Gagal load Genshin image 😿')
  }
}

handler.command = /^(gensin)$/i
handler.tags = ['asupan', 'genshin', 'waifu']
handler.help = ['gensin']
handler.limit = true
